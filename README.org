...
The search starts with the command buffer and inserts the selected command without executing it. Results initially include the current directory and its subdirectories, are grouped by command and directory, and are restricted to the current host.
